...
uint8_t g_uart1_dma_tx_buffer[UART1_TX_BUFFER_SIZE] __attribute__ ((aligned (4)));
uint8_t g_uart1_dma_rx_buffer[UART1_RX_BUFFER_SIZE] __attribute__ ((aligned (4)));
...
void uart1_send_package(uint8_t *buffer, uint32_t length)
{
	...
	if (xTaskGetSchedulerState() == taskSCHEDULER_NOT_STARTED) { /* task schedule not start */
		...
		if (g_uart1_tx_is_transfer == false)  {
			...
			memcpy(g_uart1_dma_tx_buffer, buffer, length);
			__enable_irq();
			UART1_DMA_Enable(DMA1_Channel4, length);
			va_end (args);
		} else {
